...
void AuthMsg_callback1_GECU2VCS(char* msg){
    ...
    cJSON *qrng_number = cJSON_GetObjectItem(configjson, "QRNG_number");
    const char* qrng_string = qrng_number->valuestring;
    strncpy(PW, qrng_string, 32);
    strncpy(PQCG, qrng_string + 32, 32);
    ...
    strcat((char*)MixM_inbuff, PW);
    strcat((char*)MixM_inbuff, PQCG);
    ...
    bytes_to_hex(M1, m1_bytes, 16);

    printf("M1: %s\n", M1);
    cJSON *M1_r = cJSON_GetObjectItem(json, "M1");
    printf("M1_r: %s\n", M1_r->valuestring);
    if (strncmp(M1, M1_r->valuestring, 6) == 0) {
        printf("======== M1验证成功. ========\n");
    }
    else{
        printf("======== M1未验证通过，存在问题. ========\n");
...
void AuthMsg_callback2_GECU2VCS(char* msg){
    ...
    cJSON *GID = cJSON_GetObjectItem(configjson, "GID");
    char M1[33] = {0};
    unsigned char hash_bytes[16] = {0};
    unsigned char gid_bytes[16] = {0};
    unsigned char m1_bytes[16] = {0};
    hex_to_bytes(hash_bytes, hash, 16);
    hex_to_bytes(gid_bytes, GID->valuestring, 16);
    
    for(int i = 0; i < 16; i++) 
    {
        m1_bytes[i] = hash_bytes[i] ^ gid_bytes[i];
    }
    ...
    if (strncmp(M1, M_1->valuestring, 6) == 0) {
        printf("======== M_1 验证成功. ========\n");
    }
    else{
    ...
    strcat((char*)hash_inbuf2, session_qgc);
    strcat((char*)hash_inbuf2, gid_bytes);
    sha256(hash_inbuf2,strlen(hash_inbuf2),hash_outbuff2);
    ByteToString(hash_outbuff2,hash2,32);

    /*****异或计算M2*****/
    char M_2[33] = {0};
    unsigned char hash2_bytes[16] = {0};
    unsigned char pqcg_bytes[16] = {0};
    unsigned char m2_bytes[16] = {0};
    hex_to_bytes(hash2_bytes, hash2, 16);
    hex_to_bytes(pqcg_bytes, PQCG, 16);
   
    for(int i = 0; i < 16; i++) 
    {
        m1_bytes[i] = hash2_bytes[i] ^ pqcg_bytes[i];
    }    
    bytes_to_hex(M_2, m2_bytes, 8);
...
// 2. 修正: 使用字节级异或反解 H(EIDi) 的前16字节
    ...
    if (strcasecmp(calculated_c1_hex, C1->valuestring) != 0)
    {
        ...
        MQTTClient_deliveryToken token;
        MQTTClient_publishMessage(client, "innetwork/vcs2gecu", &pubmsg, &token);
        // printf("  Sent Failed response to GECU: %s\n", out_jsonStr);
        printf("  Sent Failed response to GECU \n");
        MQTTClient_waitForCompletion(client, token, TIMEOUT);
    ...
    else
    {
        ...
        printf("  Sent SUCCESS response to GECU\n");
        MQTTClient_waitForCompletion(client, token, TIMEOUT);
...
int msgarrvd(void *context, char *topicName, int topicLen, MQTTClient_message *message) {
    ...
    else if(receiveflag == 2 || receiveflag == 3 || receiveflag == 4 || receiveflag == 5 || receiveflag == 6){
