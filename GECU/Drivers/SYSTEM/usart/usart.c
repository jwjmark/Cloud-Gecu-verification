...
int fputc(int ch, FILE *f)
{
    ...
    USART1
	->DR = (uint8_t)ch;                       /* 将要发送的字符 ch 写入到DR寄存器 */
    return ch;
}
#endif
/***********************************************END*******************************************/
...
void usart3_init(uint32_t baudrate)
{
    ...
    HAL_UART_Init(&g_uart3_handle);                             /* HAL_UART_Init()会使能UART2 */
...
void HAL_UART_MspInit(UART_HandleTypeDef *huart)
{
	...
	else if(huart->Instance==USART3)
	{
        ...
        gpio_init_struct.Pin = GPIO_PIN_10;               /* TX引脚 */
        gpio_init_struct.Mode = GPIO_MODE_AF_PP;                /* 复用推挽输出 */
        gpio_init_struct.Pull = GPIO_NOPULL;                    
        gpio_init_struct.Speed = GPIO_SPEED_FREQ_HIGH;          /* 高速 */
        gpio_init_struct.Alternate = GPIO_AF7_USART3;          /* 复用为USART3 */
        HAL_GPIO_Init(GPIOB, &gpio_init_struct);   /* 初始化发送引脚 */

        gpio_init_struct.Pin = GPIO_PIN_11;               /* RX引脚 */
        gpio_init_struct.Alternate = GPIO_AF7_USART3;          /* 复用为USART3 */
//		gpio_init_struct.Mode=GPIO_MODE_INPUT;
        HAL_GPIO_Init(GPIOB, &gpio_init_struct);   /* 初始化接收引脚 */

        HAL_NVIC_EnableIRQ(USART3_IRQn);                      /* 使能USART3中断通道 */
        HAL_NVIC_SetPriority(USART3_IRQn,1,0);              /* 抢占优先级0，子优先级0 */
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	else if(huart->Instance == USART3)             /* 如果是串口3 */
    {
		...
		{
                if((receive_buf[receive_count - 4] != 'O' && receive_buf[receive_count - 3] != 'K')||(receive_buf[receive_count - 2] != 0x22 && receive_buf[receive_count - 1] != 0x7D)) //判断是否收到ESP8266的所有消息
                {
